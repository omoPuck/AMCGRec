...
def get_evaluation_sample(training_matrix, ground_truth_matrix,negativate_len = 99):
    samples = {}
    for i in range(training_matrix.shape[0]):
        user_sample = []
        neg_indices = np.where(ground_truth_matrix[i] == 0)[0]
        # print("{:4f}neg_indices".format(i),neg_indices)
        # testing indices for the current user
        training_indices = np.where(training_matrix[i] > 0)[0]
        # print("train_indices",training_indices)
        testing_indices = [x for x in np.where(ground_truth_matrix[i] > 0)[0] if x not in training_indices]
        # print("test_indices",testing_indices)
        for test_item in testing_indices:
            neg = np.random.choice(np.array(neg_indices), negativate_len, replace=False)
            indices = np.append(neg, test_item)
            user_sample.append(indices)
        samples[i] = np.array(user_sample)
    return samples

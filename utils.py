...
def load_data(user, item):
    ...
    # np.log1p 取了个log，(把负数变成0) ???1变成0.6931472 ???0变成0 有啥用？
...
def construct_feed_dict_710(placeholders, features_user, features_item, rating, biases_list_user,
                        biases_list_item, negative,test_negative,dropout):
    print(len(biases_list_item))
    print(biases_list_item.shape)
    print(len(biases_list_user))
    print(biases_list_user.shape)
    feed_dict = dict()
    feed_dict.update({placeholders['rating']: rating})
    feed_dict.update({placeholders['features_user']: features_user})
    feed_dict.update({placeholders['features_item']: features_item})
    ...
    feed_dict.update({placeholders['support_item'][i]: biases_list_item[i] for i in range(len(biases_list_item))})
    feed_dict.update({placeholders['negative']: negative})
    feed_dict.update({placeholders['test_negative']: test_negative})
    feed_dict.update({placeholders['dropout']: dropout})
    return feed_dict


def get_cosine_similarity(user_emb, item_emb):
    user_emb = tf.nn.l2_normalize(user_emb, axis=2)
    item_emb = tf.nn.l2_normalize(item_emb, axis=2)
    # print(user_emb.shape)
    # print(item_emb.shape)

    #tf.multiply(user_emb, item_emb).shape = (batch_size, item_size, embedding_size)
    return tf.reduce_sum(tf.multiply(user_emb, item_emb),axis=2)
...
def new_softmax(pos_cos,neg_cos,tua):
    mol = tf.exp(pos_cos/tua)
    temp = tf.concat([tf.exp(neg_cos / tua), mol], axis = 1)
    den = tf.reduce_sum(temp,1)
    return mol/den